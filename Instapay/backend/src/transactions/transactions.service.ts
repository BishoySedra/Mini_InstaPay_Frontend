...
export class TransactionsService {
  ...
  async makeTransaction(
    ...
    if (senderAccount.userId !== user.id) {
      throw new BadRequestException('You do not own this sender account');
    }
    ...
    if (receiverAccount.userId === senderAccount.userId) {
      throw new BadRequestException('You cannot send money to your own account');
    }
  ...
  async acceptRefund(transactionId, user) {
    {
      return await this.prisma.$transaction(async (prisma) => {
        ...
        if (transaction.type === 'REFUND')
          throw new UnauthorizedException('transaction already refunded');
        ...
        if (transaction.receiverId != user.id)
          throw new BadRequestException('you can only accept your refund');
